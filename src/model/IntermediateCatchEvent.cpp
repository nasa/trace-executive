...
IntermediateCatchEvent::IntermediateCatchEvent(const std::string &uuid,
...
void IntermediateCatchEvent::start() {
  if (event_reference_ != nullptr) {
    signal_connection_ = event_reference_->attachEventResultCallback(
        boost::bind(&IntermediateCatchEvent::event_callback, this,
                    boost::placeholders::_1));
  }
  Event::start(); // ALWAYS CALL THIS AT THE END!
}

Outcome IntermediateCatchEvent::activity() {
...
void IntermediateCatchEvent::cleanup() { signal_connection_.disconnect(); }

void IntermediateCatchEvent::event_callback(
  ...
  TRACE_LOG_INFO(uuid(), "Caught event (" << event->getEventUuid() << ").");
}
